...
from ..models.chart_data import (
    FileTypeData, DirectoryNode, FileDistributionData, 
    TopFilesData, FileAgeData, ChartMetadata, ChartDataTransformer
)
...
class VisualizationDataService:
    ...
    def get_file_type_data(self) -> List[FileTypeData]:
        ...
        for item in type_data:
            item.color = color_map.get(item.type, color_map.get("OTHER", "#bdc3c7")).name()
            
        return type_data
    
    def get_directory_hierarchy(self) -> Optional[DirectoryNode]:
    ...
    def get_file_size_distribution(self) -> FileDistributionData:
        ...
        percentages = [count / total_files * 100 if total_files > 0 else 0 
    ...
    def filter_by_size_range(self, min_size: int, max_size: int) -> List[Dict[str, Any]]:
