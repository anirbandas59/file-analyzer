...
from ..models.chart_data import FileTypeData, ChartMetadata
    ...
    def draw_pie_segments(self, painter: QPainter, chart_rect: QRect):
        ...
        for i, item in enumerate(self.data):
            ...
            if item.percentage > 5:  # Only show text for segments > 5%
                self.draw_segment_label(painter, chart_rect, start_angle, span_angle, item)
            
            start_angle += span_angle
            
    def draw_segment_label(self, painter: QPainter, chart_rect: QRect, 
                          start_angle: int, span_angle: int, item: FileTypeData):
        ...
        label_y = center_y - label_radius * math.sin(mid_angle_rad)  # Negative because Y is inverted
...
class FileTypePieChart(InteractiveChart):
    ...
    def update_data(self, data: List[FileTypeData], metadata: Optional[ChartMetadata] = None):
    ...
    def on_segment_clicked(self, data_item: FileTypeData):
        ...
        self.emit_item_clicked(data_item.type, {
    ...
    def get_item_at_position(self, x: int, y: int) -> Optional[Dict[str, Any]]:
