...
class ManagementOverview(QWidget):
    ...
    def create_tool_card(self, title: str, description: str, details: str, tool_id: str) -> CardWidget:
    ...
    def setup_quick_stats(self, layout):
        ...
        stats_card = TitleCard("Quick Statistics", "Analysis results will appear here after running tools")
...
class ManagementDashboard(QWidget):
    ...
    def update_data(self, files: List[Dict], directory_path: str = ""):
        ...
        if hasattr(self, 'large_files_tool'):
            self.large_files_tool.update_files(files)
            
        if hasattr(self, 'age_analysis_tool'):
            self.age_analysis_tool.update_files(files)
            
        # Update overview stats if duplicate analysis has been run
        if hasattr(self.duplicate_finder, 'duplicate_groups') and self.duplicate_finder.duplicate_groups:
    ...
    def get_management_summary(self) -> Dict:
        ...
        if self.duplicate_finder and hasattr(self.duplicate_finder, 'duplicate_groups'):
            ...
            summary['total_potential_savings'] += duplicate_stats.get('potential_savings', 0)
