...
from ..models.management_data import (
    FileInfo, LargeFileAnalysis, FileSizeCategory
)
...
class LargeFileAnalysisService:
    ...
    def analyze_large_files(self, files: List[Dict], 
                          sort_by: str = "size",
                          sort_ascending: bool = False) -> LargeFileAnalysis:
        """
        ...
        if sort_by == "size":
            ...
            large_files.sort(key=lambda f: f.file_type.lower(), reverse=not sort_ascending)
    ...
    def get_size_distribution(self, files: List[FileInfo]) -> Dict[FileSizeCategory, Dict]:
        """
        ...
        for category, category_files in categories.items():
            ...
            distribution[category] = {
                ...
                'percentage': (len(category_files) / total_files * 100) if total_files > 0 else 0,
                'size_percentage': (category_size / total_size * 100) if total_size > 0 else 0,
                'files': category_files
            }
        
        return distribution
    
    def get_cleanup_candidates(self, files: List[FileInfo], 
                             consider_age: bool = True,
                             min_age_days: int = 365) -> List[FileInfo]:
        """
    ...
    def generate_detailed_recommendations(self, analysis: LargeFileAnalysis) -> List[Dict]:
        """
        ...
        for file_type, data in type_analysis.items():
            if data['total_size'] > self.size_threshold:
                ...
                recommendations.append({
                    ...
                    'priority': 'high' if data['total_size'] > 1024*1024*1024 else 'medium',  # >1GB
        ...
        top_dirs = sorted(dir_analysis.items(), key=lambda x: x[1]['total_size'], reverse=True)[:3]
        ...
        huge_files = [f for f in analysis.files if f.size_category in [FileSizeCategory.HUGE, FileSizeCategory.MASSIVE]]
        if huge_files:
    ...
    def export_analysis_results(self, analysis: LargeFileAnalysis, 
    ...
    def _export_to_csv(self, analysis: LargeFileAnalysis) -> str:
        ...
        # Header
        writer.writerow([
            'File Name', 'Path', 'Size (Bytes)', 'Size (Formatted)', 
            'Size Category', 'Last Modified', 'File Type'
        ])
