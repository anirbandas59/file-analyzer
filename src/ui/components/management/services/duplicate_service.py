...
from ..models.management_data import (
    FileInfo, DuplicateGroup, DuplicateConfidence, ManagementSummary
)
...
class DuplicateDetectionService:
    ...
    def find_duplicates(self, files: List[Dict], 
                       ...
                       use_heuristics: bool = True) -> List[DuplicateGroup]:
        """
    ...
    def _find_hash_duplicates(self, files: List[FileInfo]) -> List[DuplicateGroup]:
        ...
        for i, file in enumerate(files):
            if self.progress_callback:
                self.progress_callback(f"Quick scan: {file.name}", int((i / total_files) * 50))
        ...
        for i, file in enumerate(files_to_hash):
            if self.progress_callback:
                self.progress_callback(f"Deep scan: {file.name}", 50 + int((i / len(files_to_hash)) * 50))
    ...
    def _merge_duplicate_groups(self, groups: List[DuplicateGroup]) -> List[DuplicateGroup]:
        ...
        for group in groups:
            ...
            for og in overlapping_groups:
                ...
                elif og.confidence.value == DuplicateConfidence.MEDIUM.value and best_confidence.value != DuplicateConfidence.HIGH.value:
            ...
            if len(unique_file_list) >= 2:
                merged_groups.append(DuplicateGroup(
    ...
    def calculate_space_savings(self, duplicate_groups: List[DuplicateGroup]) -> Tuple[int, int]:
        """
