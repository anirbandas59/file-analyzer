...
from ..models.management_data import LargeFileAnalysis, FileInfo, FileSizeCategory
...
class LargeFileItemWidget(CardWidget):
    ...
    def setup_ui(self):
        ...
        delete_btn.clicked.connect(lambda: self.file_action_requested.emit("delete", self.file_info.path))
...
class LargeFileAnalyzerTool(QWidget):
    ...
    def setup_header(self, layout):
        ...
        header_card = TitleCard("Large File Analyzer", "Identify and manage the largest files consuming disk space")
    ...
    def setup_configuration(self, layout):
        ...
        self.sort_combo.addItems(["Size (Largest First)", "Size (Smallest First)", 
                                 "Name", "Path", "Date Modified", "File Type"])
    ...
    def add_summary_stats(self):
        ...
        avg_size = total_size // len(self.current_analysis.files) if self.current_analysis.files else 0
        
        summary_card = CardWidget()
        summary_layout = QHBoxLayout(summary_card)
        
        files_stat = StatsCard("Large Files Found", str(len(self.current_analysis.files)))
        total_size_stat = StatsCard("Total Size", format_size(total_size))
        avg_size_stat = StatsCard("Average Size", format_size(avg_size))
        threshold_stat = StatsCard("Size Threshold", format_size(self.current_analysis.size_threshold))
    ...
    def add_recommendations(self):
        ...
        recommendations = self.service.generate_detailed_recommendations(self.current_analysis)
        
        if recommendations:
            rec_card = TitleCard("Cleanup Recommendations", "Suggested actions to optimize disk space")
            rec_content = QWidget()
    ...
    def add_large_files_list(self):
        ...
        files_card = TitleCard("Large Files", f"Files larger than {self.threshold_spinbox.value()} MB")
        ...
        if len(self.current_analysis.files) > 50:
            more_label = QLabel(f"... and {len(self.current_analysis.files) - 50} more files")
        ...
        if reply == QMessageBox.StandardButton.Yes:
            try:
                ...
                if self.current_analysis:
                    self.current_analysis.files = [f for f in self.current_analysis.files if f.path != file_path]
                    self.display_results()
    ...
    def get_analysis_summary(self) -> Dict:
        ...
        return {
            ...
            'threshold_mb': self.threshold_spinbox.value(),
            'recommendations': len(self.service.generate_detailed_recommendations(self.current_analysis))
        }