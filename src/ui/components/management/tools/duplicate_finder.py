...
class DuplicateGroupWidget(CardWidget):
    ...
    def setup_header(self, layout):
        ...
        confidence_label = QLabel(
            f"Confidence: {self.duplicate_group.confidence.value.title()}"
        )
        ...
        savings_text = (
            f"Potential savings: {format_size(self.duplicate_group.potential_savings)}"
        )
    ...
    def remove_selected_files(self):
        ...
        if not selected_files:
            QMessageBox.information(
                self, "No Selection", "Please select files to remove."
            )
            return
    ...
    def delete_files(self, files: list[FileInfo]):
        ...
        for file in files:
            try:
                ...
                failed_files.append(f"{file.name}: {e!s}")

        # Show results
        if success_count > 0:
            QMessageBox.information(
                self, "Success", f"Successfully deleted {success_count} files."
            )

        if failed_files:
            error_message = "\\n".join(failed_files)
            QMessageBox.warning(
                self, "Some Deletions Failed", f"Failed to delete:\\n{error_message}"
            )
...
class DuplicateFinderTool(QWidget):
    ...
    def setup_controls(self, layout):
        ...
        self.use_content_hash = QCheckBox(
            "Content-based detection (slower, more accurate)"
        )
        self.use_content_hash.setChecked(True)

        self.use_heuristics = QCheckBox(
        ...
        return {
            ...
            "high_confidence_groups": len(
                [
                    ...
                    if g.confidence == DuplicateConfidence.HIGH
                ]
            ),
        }
