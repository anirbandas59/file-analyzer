...
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
                             ...
                             QHeaderView, QPushButton, QFrame, QScrollArea,
...
from ..models.management_data import FileAgeAnalysis, FileInfo, FileAgeCategory
...
class AgeCategoryCard(CardWidget):
    ...
    def setup_ui(self):
        ...
        if self.stats['count'] > 0:
            ...
            view_button.clicked.connect(lambda: self.category_selected.emit(self.category))
...
class ArchivalCandidateWidget(CardWidget):
    ...
    def setup_ui(self):
        ...
        open_btn.clicked.connect(lambda: self.file_action_requested.emit("open_location", self.file_info.path))
        
        archive_btn = ModernButton("Archive", "primary")
        archive_btn.setMaximumWidth(80)
        archive_btn.clicked.connect(lambda: self.file_action_requested.emit("archive", self.file_info.path))
...
class FileAgeAnalyzerTool(QWidget):
    ...
    def setup_header(self, layout):
        ...
        header_card = TitleCard("File Age Analyzer", "Analyze file ages and get archival recommendations")
    ...
    def setup_configuration(self, layout):
        ...
        # Analysis options
        self.include_stale_checkbox = QCheckBox("Include stale files (6mo-1yr) in recommendations")
    ...
    def add_age_distribution(self):
        ...
        dist_card = TitleCard("Age Distribution", "Files categorized by last modification date")
    ...
    def add_archival_recommendations(self):
        ...
        if recommendations:
            rec_card = TitleCard("Archival Recommendations", "Suggested actions based on file age analysis")
            rec_content = QWidget()
    ...
    def add_archival_candidates(self):
        ...
        if candidates:
            candidates_card = TitleCard("Archival Candidates", f"{len(candidates)} files ready for archival")
            ...
            if len(candidates) > 25:
                more_label = QLabel(f"... and {len(candidates) - 25} more archival candidates")
    ...
    def on_category_selected(self, category: FileAgeCategory):
        ...
        if category_files:
            ...
            file_list = "\\n".join([f"• {f.name} ({f.path})" for f in category_files[:10]])
    ...
    def select_all_candidates(self):
        ...
        QMessageBox.information(self, "Select All", "All visible archival candidates have been selected.")
    
    def select_no_candidates(self):
        """Deselect all archival candidates."""
        self.selected_files.clear()
        QMessageBox.information(self, "Select None", "All archival candidates have been deselected.")
    ...
    def archive_selected_files(self):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            QMessageBox.information(self, "Archive Feature", "Archive functionality will be implemented in a future version.")
    ...
    def archive_single_file(self, file_path: str):
        ...
        QMessageBox.information(self, "Archive Feature", "Individual file archiving will be implemented in a future version.")
    ...
    def export_results(self):
        ...
        try:
            ...
            export_data = self.service.export_analysis_results(self.current_analysis, format_type)
            
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(export_data)
                
            QMessageBox.information(self, "Export Successful", f"Age analysis results exported to:\\n{file_path}")
    ...
    def get_analysis_summary(self) -> Dict:
        ...
        return {
            ...
            'recommendations': len(self.service.get_archival_recommendations(self.current_analysis))
        }