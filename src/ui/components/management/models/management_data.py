...
class FileInfo:
    ...
    hash_partial: str | None = None  # First 1KB hash for quick comparison
    size_category: FileSizeCategory | None = None
    age_category: FileAgeCategory | None = None
...
class DuplicateGroup:
    ...
    def __post_init__(self):
        ...
        self.potential_savings = (
            self.total_size - max(file.size for file in self.files) if self.files else 0
        )
    ...
    def get_recommended_keeper(self) -> FileInfo | None:
...
class LargeFileAnalysis:
    ...
    def get_files_by_category(self) -> dict[FileSizeCategory, list[FileInfo]]:
        ...
        for category in FileSizeCategory:
            categories[category] = [
                f for f in self.files if f.size_category == category
            ]
        return categories
    ...
    def get_cleanup_recommendations(self) -> list[str]:
        ...
        for file_type, size in sorted_types[:3]:  # Top 3 types
            ...
            recommendations.append(
                f"Consider reviewing {file_type} files ({format_size(size)} total)"
            )
...
class FileAgeAnalysis:
    ...
    def get_size_by_age(self) -> dict[FileAgeCategory, int]:
        ...
        return {
            category: sum(f.size for f in files)
    ...
    def get_archival_recommendations(self) -> list[str]:
        ...
        if archive_size > 0:
            ...
            archive_count = len(
                self.get_files_by_age().get(FileAgeCategory.ARCHIVE, [])
            )
            recommendations.append(
...
class ManagementSummary:
    ...
    large_files: LargeFileAnalysis | None = None
    age_analysis: FileAgeAnalysis | None = None
    ...
    def __post_init__(self):
        ...
        self.potential_space_savings = sum(
            group.potential_savings for group in self.duplicate_groups
        )
    ...
    def get_top_recommendations(self) -> list[str]:
        ...
        if self.duplicate_groups:
            ...
            duplicate_count = sum(
                len(group.files) - 1 for group in self.duplicate_groups
            )
